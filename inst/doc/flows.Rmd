...
The **flows** package contains functions to filter flows matrices, provides statistics on selection made and proposes map and graph visualisation.    
The first part of the vignette reminds several methods of flows selection, the second part presents the main functions of the package and the last one proposes an exemple of analysis based on commuters data in the French Grand Est region.  

#Analysis of geographic flows: issues and methods

In spatial analysis field, working on flows supposes to focus on the relationships between places rather than on their characteristics. The analysis and especially the representation of flows often assume a selection to ease interpretation. 
...
This method creates what is called in graph theory a tree (acyclic graph) or a forest (a set of unconnected trees) with three types of nodes: dominant, dominated and intermediate nodes. If the method creates a clear functional hierarchy, its major drawback is to undervalue flows intensities.

Various methods have subsequently been proposed to better reflect this intensity, one of the most frequently used is the so-called major flows: it selects only the most important flows, absolute or relative, either locally or globally. Analysing commuters data between cities, one may choose to select:

* all flows greater than 100;

* the 50 first flows (global criterion);

* the 10 first flows emitted by each city (local criteria).

These criteria can also be expressed in relative form:

* the flows that represent more than 10% of the active population of each city (local criteria);
...
A data workflow may be as follow:
...
4. graph or map representation (dominant flow).

##Data Preparation
The input data can be in a long format (_i-j-fij_), i.e. origin - destination - flow intensity. The `preflows` function transforms the links list into a square matrix (wide format). All selection functions take a square matrix with the same list of origins and destinations as input. `preflows` takes four arguments: a data.frame to transform (`mat`), the origin (`i`), the destination (`j`) and the flow intensity (`fij`).
...
##Flow Selection
Three selection methods based on the flows origins are accessible through the `firstflows` function:

* `nfirst`: `k` first flows from all origins;
...
Methods taking into account the total volume of flows are implemented in the `firstflowsg` function. They are identical to ones described above: selection of the `k` first flows, selection of flows greater than `k` and  selection of flows such as the sum is at least equal to `k`.



Finally, the function `domflows` selects flows based on a dominance test. This function may be used to select flows obeying the second criterion of Nystuen and Dacey method.




All these functions take as input a square flow matrix and generate binary matrices of equal size. The flows selected by the method are represented by 1, the other by 0. It is therefore possible to combine criteria of selection through element-wise multiplication of matrices (Figure 2).

Figure 2: Flow selection and criteria combination
 <img src="figure2en.png" width="680px"  />

It is possible to measure a number of indicators using the `statmat` function. Measures provided are density (number of present flows divided by the number of possible flows); number, size and composition of connected components; sum, quartiles and average intensity of flows. It is therefore possible to make a statistically relevant selection. This function can also generate four graphics outputs: the degree distribution (by default, outdegree), the weighted degree distribution, a Lorenz curve and a boxplot of the flows intensity.
...
Visualisations helps analysis, `plotDomFlows` function produces a graph where sizes and colors of vertices depend on their position in the graph (dominant, intermediate or dominated) and links thickness depend on flow intensity.    
...
As an illustration, we present a brief analysis on commuter flows between the urban areas of Grand Est region in France[^2]. 
...
If we select flows greater than 500 commuters, we loose 95.7% of all links but only 38% of the volume of flows. With a threshold of 1000, 98% of links are lost but only 53% of the volume of flows.
...
We decide to use this selection as first criteria of our analysis. The second one will be a dominant flow selection based on sum of incoming flows.
...
The top of the node hierarchy evaluated by the selection of dominant flow and number of incoming commuters brings out clearly, in descending order, the domination of Strasbourg, Mulhouse, Metz and Nancy, each attracting more than 10 000 commuters.
