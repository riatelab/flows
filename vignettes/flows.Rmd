...
The `dominant` method selects flows based on a dominance test. This function may be used to select flows obeying the second criterion of Nystuen and Dacey method.
...
Visualisation helps analysis, `plot_nodal_flow()` function produces a graph where sizes and colors of vertices depend on their position in the graph (dominant, intermediate or dominated) and thicknesses of links depend on flow intensites.    
...
`nodal_flows()` combines the two critiria in a single function and returns a flow matrix.
...
This analysis keeps 4% of all links and 4% ot the flows volume. 
